...
export default function RostersList({ rosters }: RostersListProps) {
  ...
  return (
    <div className={styles.wrapper}>
          ...
          >
            ...
            <div className={styles.cardFooterActions}>
              <button
                ...
                onClick={(e) => handleDuplicate(roster.id, e)}
                disabled={duplicatingId === roster.id}
              ...
              >
                ...
                {duplicatingId === roster.id ? 'Copying…' : 'Copy to next week'}
              </button>
            </div>
          </div>
        ))}
      ...
      {error && !showConfirm && (
        <div className={styles.error} role="alert" style={{ marginTop: 12 }}>{error}</div>
      )}
    </div>
  );
}
